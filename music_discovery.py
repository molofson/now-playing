...
class DiscoveryApp:
    ...
    def _setup_enrichment(self):
        ...
        def on_enrichment_complete(enrichment_data, context):
            """Handle completion of enrichment callbacks."""
    ...
    def _on_metadata(self, metadata: dict) -> None:
        """Process and apply metadata updates."""
        try:
...
def load_user_panels(config: AppConfig) -> None:  # noqa: U100 - config will be used when plugin loading is implemented
...
def validate_api_tokens() -> bool:
    """Validate required API tokens and test their validity."""
    import os
    ...
    required_tokens = {
        "DISCOGS_API_TOKEN": "https://www.discogs.com/settings/developers",
        "LASTFM_API_KEY": "https://www.last.fm/api/account/create",
        "SETLISTFM_API_KEY": "https://api.setlist.fm/docs/1.0/index.html",
    }

    optional_tokens = {"GENIUS_API_KEY": "https://genius.com/developers"}

    # Check required tokens
    present_required = []
    missing_required = []
    for token_name, signup_url in required_tokens.items():
        if os.environ.get(token_name):
            present_required.append(token_name)
        else:
            missing_required.append(f"{token_name} (get from {signup_url})")

    # Check optional tokens
    present_optional = []
    missing_optional = []
    for token_name, signup_url in optional_tokens.items():
        if os.environ.get(token_name):
            present_optional.append(token_name)
        else:
            missing_optional.append(f"{token_name} (optional, get from {signup_url})")

    # Report results
    if present_required:
        logger.info("Required API tokens found:")
        for token in present_required:
            logger.info(f"  [OK] {token}")

    if present_optional:
        logger.info("Optional API tokens found:")
        for token in present_optional:
            logger.info(f"  [OK] {token}")

    if missing_required:
        logger.error("Missing required API tokens:")
        for token in missing_required:
            logger.error(f"  [MISSING] {token}")
        return False

    if missing_optional:
        logger.warning("Missing optional API tokens:")
        for token in missing_optional:
            logger.warning(f"  [MISSING] {token}")

    # List services that don't require tokens
    logger.info("Services available without API tokens:")
    logger.info("  [FREE] MusicBrainz - Core music metadata and cross-service linking")
    logger.info("  [FREE] Songfacts - Song lyrics and background stories")
    logger.info("  [FREE] AllMusic - Professional music reviews and credits")
    logger.info("  [FREE] Pitchfork - Music criticism and reviews")

    # Test token validity by creating enrichment engine
    try:
        logger.info("Testing token validity with enrichment engine...")
        EnrichmentEngine()  # Test that engine can be created with current tokens
        # The engine will log warnings for invalid tokens during initialization
...
def setup_signal_handlers(app: DiscoveryApp) -> None:
    ...
    def signal_handler(signum, frame):  # noqa: U100 - frame required by signal handler signature
...
def main():
    """Run the music discovery application."""
    parser = argparse.ArgumentParser(
        description="Music Discovery - AirPlay metadata display with enrichment panels",
        epilog="""
Examples:
  %(prog)s                                    # Run in kiosk mode (default)
  %(prog)s --windowed                         # Run in windowed mode for development
  %(prog)s --fullscreen                       # Run in fullscreen mode
  %(prog)s --capture session.json             # Capture metadata for testing
  %(prog)s --replay session.json.gz           # Replay captured session
...
Services available without tokens:
  MusicBrainz, Songfacts, AllMusic, Pitchfork

Controls:
  Left/Right arrows    - Navigate between panels
  Space                - Hold/release context
  F11                  - Cycle display modes
  ...
  Q/Esc                - Quit application
    ...
    parser.add_argument(
        "--windowed",
        action="store_true",
        help="Run in windowed mode (1200x800, with decorations) - recommended for development",
    )
    parser.add_argument(
        "--fullscreen", action="store_true", help="Run in fullscreen mode (uses full display resolution)"
    )
    parser.add_argument(
        "--kiosk",
        action="store_true",
        help="Run in kiosk mode (fullscreen, hides taskbars, default) - recommended for production",
    )
    parser.add_argument("--debug", action="store_true", help="Enable debug logging and additional debug panels")
    parser.add_argument(
        "--capture", metavar="FILE", help="Capture metadata to FILE for testing/replay (creates JSONL format)"
    )
    parser.add_argument(
        "--replay", metavar="FILE", help="Replay metadata from FILE with fast-forward (supports .gz compression)"
    )
    parser.add_argument(
        "--validate-tokens", action="store_true", help="Validate that API tokens are set and test their validity"
    )
