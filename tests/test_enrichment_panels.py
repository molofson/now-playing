...
class TestEnrichmentPanels:
    ...
    def test_panel_display_logic(self, sample_context):
        ...
        # Test with empty context (no metadata)
        empty_context = ContentContext()
        assert not panel.can_display(empty_context)
...
class TestEnrichmentDataAttachment:
    ...
    def test_context_without_enrichment_data(self):
        ...
        # enrichment_data attribute should exist but be None initially
        assert hasattr(context, "enrichment_data")
        assert context.enrichment_data is None
